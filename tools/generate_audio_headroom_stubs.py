...
NOISE_INSTRUMENT_FILE_RE = re.compile(r"^noise_instrument[0-9][0-9]\.asm$")
...
def write_stub(out_path: Path, source_files: list[str], *, expand_all_engines: bool = False) -> None:
    ...
    for rel in source_files:
        ...
        for base, channel in channels:
            ...
            if expand_all_engines:
                for engine in (1, 2, 3):
                    lines.append(f"{base}_{engine}_Ch{channel}::")
                    lines.append("\tsound_ret")
            else:
...
def all_audio_payload_files() -> list[str]:
    # Includes every music/sfx payload file so a single stub include can patch out all audio data.
    files = sorted(Path("audio/music").glob("*.asm")) + sorted(Path("audio/sfx").glob("*.asm"))
    result: list[str] = []
    for path in files:
        if NOISE_INSTRUMENT_FILE_RE.match(path.name):
            continue
...
def main() -> int:
    ...
    all_audio = all_audio_payload_files()
    ...
    write_stub(OUT_DIR / "all_audio_stubs.asm", all_audio, expand_all_engines=True)
    ...
    print("wrote audio/headroom/all_audio_stubs.asm")
    return 0
